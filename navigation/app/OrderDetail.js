...
export default class OrderDetailScreen extends React.Component {
  render() {
    ...
    const voucher = type === 'Invoice' ? 'Factura' : 'Pedido';

    let content;
    let subtotal = 0;
    if (type === 'Invoice') {
      subtotal = data.get_total;
      const total = data.get_total + data.iva + data.taxes;
      content = (
        <View>
          <View style={styles.title}>
            <Text style={styles.titleText}>DETALLE DEL COMPROBANTE</Text>
          </View>
      ...
      subtotal = data.get_total;
      const iva = (data.get_total * 21) / 100;
      const total = subtotal + iva;
      const shipping =
        data.shipping === 'ENV' ? 'Envío a Domicilio' : 'Retiro en Local';
      content = (
        <View>
          <View style={styles.title}>
            <Text style={styles.titleText}>DETALLE DEL PEDIDO</Text>
          </View>
          <View style={styles.dataContainer}>
            <View style={styles.header}>
              <Text style={styles.headerText}>
                {voucher} Nº: {data.order_id}
              </Text>
              <Text style={styles.headerText}>
                Fecha: {format(parse(data.date), 'DD/MM/YY')}
              </Text>
            ...
            <View style={styles.productList}>
              <OrderDetailProductsTable products={data.items} type={'Order'} />
            </View>
            <View style={styles.totalsContainer}>
              <View style={styles.totalRow}>
                <Text style={styles.totalText}>Subtotal:</Text>
              ...
              <View style={styles.deliveryMethod}>
                ...
                <View style={styles.payData}>
                  ...
                  <Text style={styles.payText}>{shipping}</Text>
                </View>
              </View>
            </View>
          </View>
